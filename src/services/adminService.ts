...
export const adminService = {
  ...
  // REPARAT: Adăugată logica pentru a converti "" sau "none" în null pentru p_entity_id
  async createUserAccountAndMap(email: string, password: string, role: AppRole, entityId: string) {
    const { data: newUserId, error } = await supabase.rpc('admin_create_user_and_profile', {
      ...
      p_password: password,
      p_role: role,
      // Verificăm dacă entityId este valid; dacă nu, trimitem null pentru a respecta constrângerile UUID din SQL
      p_entity_id: (entityId && entityId !== 'none') ? entityId : null
    });
